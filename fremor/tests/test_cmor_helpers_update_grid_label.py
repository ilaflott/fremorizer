...
from fremor.cmor_helpers import update_grid_and_label
...
def test_missing_grid_label_field(test_json_file):
    ...
    # Act & Assert
    with pytest.raises(KeyError, match='Error while updating grid_label. Ensure the field exists and is modifiable.'):
        update_grid_and_label(test_json_file, new_grid_label, new_grid, new_nom_res)
...
def test_missing_grid_field(test_json_file):
    ...
    # Act & Assert
    with pytest.raises(KeyError, match='Error while updating grid. Ensure the field exists and is modifiable.'):
        update_grid_and_label(test_json_file, new_grid_label, new_grid, new_nom_res)
